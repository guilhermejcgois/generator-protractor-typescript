...
module.exports = class extends Generator {
  prompting() {
    ...
    this.log(yosay(
      'Welcome to the fantastic ' + chalk.red('generator-protractor-typescript') + ' generator!'
    ));
  ...
  writing() {
    // const AddProductLinkText = 'Adicionar produto';
    // const DashboardLinkSelector = 'a[ng-reflect-router-link="resumo"]';
    ...
    props.elementsInfo.forEach(info => {
      ...
      constructorStatements.push(`this.${elementVar} = ${elementFindMethod}(${byMethod}(${selector}))`);
    });
    const procReplacer = {
      process: function(content) {
        let newContent = content.toString().split('/* Elements Selector */').join(selectors.join('\n\t'));
        newContent = newContent.toString().split('/* Members Declaration */').join(declarations.join('\n\t'));
        newContent = newContent.toString().split('/* Constructor Body */').join(constructorStatements.join('\n\t\t'));
        newContent = newContent.toString().split('/* Class Name */').join(props.file.className);
        return newContent;
      }
    };
